import { CustomersOfSalesmenClient } from './client';
import {
  CustomersOfSalesmen,
  CustomersOfSalesmenField,
  CustomersOfSalesmenSortSpec,
} from './types';
import { ApiClientConfig } from '../../types';

describe('CustomersOfSalesmenClient', () => {
  let client: CustomersOfSalesmenClient;
  ...
  beforeEach(() => {
    ...
    client = new CustomersOfSalesmenClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all customersOfSalesmen', async () => {
      const mockResponse = {
        items: [
          {
    ...
    it('should get all customersOfSalesmen with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['CODE'] as CustomersOfSalesmenSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/customersOfSalesmen?limit=10&offset=0&sort=CODE'
      );
    });

    it('should get customersOfSalesmen by ID', async () => {
      const mockCustomersOfSalesmen: CustomersOfSalesmen = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
        TYPE: 0,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockCustomersOfSalesmen);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/customersOfSalesmen/1');
      expect(result).toEqual(mockCustomersOfSalesmen);
    });

    it('should get customersOfSalesmen by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/customersOfSalesmen/1?expandLevel=full'
      );
    });

    it('should create a new customersOfSalesmen', async () => {
      const newCustomersOfSalesmen = {
        CODE: 'Test CODE',
        NAME: 'Test NAME',
        TYPE: 0,
      };
      const createdCustomersOfSalesmen: CustomersOfSalesmen = {
        ...newCustomersOfSalesmen,
        INTERNAL_REFERENCE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdCustomersOfSalesmen);

      const result = await client.create(newCustomersOfSalesmen);
      ...
      expect(result).toEqual(createdCustomersOfSalesmen);
    });

    it('should update a customersOfSalesmen', async () => {
      const updateCustomersOfSalesmen = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
        TYPE: 0,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateCustomersOfSalesmen);

      const result = await client.update(1, updateCustomersOfSalesmen);

      expect(client['request']).toHaveBeenCalledWith(
        'put',
        '/customersOfSalesmen/1',
        updateCustomersOfSalesmen
      );
      expect(result).toEqual(updateCustomersOfSalesmen);
    });

    it('should patch a customersOfSalesmen', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('patch', '/customersOfSalesmen/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a customersOfSalesmen', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
  ...
  describe('Search Methods', () => {
    ...
    it('should search customersOfSalesmen', async () => {
  ...
  describe('Analytics Methods', () => {
    it('should get customersOfSalesmen analytics', async () => {
    ...
    it('should get customersOfSalesmen count', async () => {
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: CustomersOfSalesmen not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: CustomersOfSalesmen not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    ...
    it('should call importFromXmlStrV1 with query options', async () => {
      const options = { limit: 10, fields: ['CODE', 'NAME'] as CustomersOfSalesmenField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
