import { FreeZonesClient } from './client';
import { FreeZones, FreeZonesField, FreeZonesSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('FreeZonesClient', () => {
  let client: FreeZonesClient;
  ...
  beforeEach(() => {
    ...
    client = new FreeZonesClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all freeZones', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones');
      expect(result).toEqual(mockResponse);
    });

    it('should get all freeZones with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['COUNTRY_NO'] as FreeZonesSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/freeZones?limit=10&offset=0&sort=COUNTRY_NO'
      );
    });

    it('should get freeZones by ID', async () => {
      const mockFreeZones: FreeZones = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockFreeZones);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/1');
      expect(result).toEqual(mockFreeZones);
    });

    it('should get freeZones by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/1?expandLevel=full');
    });

    it('should create a new freeZones', async () => {
      const newFreeZones = {
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      const createdFreeZones: FreeZones = { ...newFreeZones, INTERNAL_REFERENCE: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdFreeZones);

      const result = await client.create(newFreeZones);

      expect(client['request']).toHaveBeenCalledWith('post', '/freeZones', newFreeZones);
      expect(result).toEqual(createdFreeZones);
    });

    it('should update a freeZones', async () => {
      const updateFreeZones = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateFreeZones);

      const result = await client.update(1, updateFreeZones);

      expect(client['request']).toHaveBeenCalledWith('put', '/freeZones/1', updateFreeZones);
      expect(result).toEqual(updateFreeZones);
    });

    it('should patch a freeZones', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('patch', '/freeZones/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a freeZones', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/freeZones/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search freeZones', async () => {
  ...
  describe('Analytics Methods', () => {
    it('should get freeZones analytics', async () => {
    ...
    it('should get freeZones count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/freeZones/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: FreeZones not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: FreeZones not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    ...
    it('should call equalizePayAmntV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });

    it('should call equalizePayAmntV1Get with query options', async () => {
      const options = { limit: 10, fields: ['COUNTRY_NO', 'CODE'] as FreeZonesField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/AttachADespatchByLRef/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/DeleteCampaign`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/SetClientInfo`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/GetRelevantCampaigns`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ImportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ExportToXML/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/CreateCompositeLines`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/FormSeriLotLines/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/ApplyCampaign`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ApplyRePayPln/1/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ExportToXmlStr/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/ApplyCondition`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ExportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/ReCalculate`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/freeZones/FillAccCodes`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/freeZones/1/ApplyADiscount/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/freeZones/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/freeZones/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
