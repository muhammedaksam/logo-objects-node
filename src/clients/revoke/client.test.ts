...
import { Revoke, RevokeField, RevokeSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('RevokeClient', () => {
  let client: RevokeClient;
  ...
  beforeEach(() => {
    ...
    client = new RevokeClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all revoke', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/revoke');
      expect(result).toEqual(mockResponse);
    });

    it('should get all revoke with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['INTERNAL_REFERENCE'] as RevokeSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/revoke?limit=10&offset=0&sort=INTERNAL_REFERENCE'
      );
    });
  });
  ...
  describe('Analytics Methods', () => {
    it('should get revoke analytics', async () => {
    ...
    it('should get revoke count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/revoke/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/revoke/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/revoke/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/revoke/checktrack');
