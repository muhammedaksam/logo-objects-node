import { ExtendedFieldsClient } from './client';
import { ExtendedFields, ExtendedFieldsField, ExtendedFieldsSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('ExtendedFieldsClient', () => {
  let client: ExtendedFieldsClient;
  ...
  beforeEach(() => {
    ...
    client = new ExtendedFieldsClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all extendedFields', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields');
      expect(result).toEqual(mockResponse);
    });

    it('should get all extendedFields with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['MODULENR'] as ExtendedFieldsSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/extendedFields?limit=10&offset=0&sort=MODULENR'
      );
    });

    it('should get extendedFields by ID', async () => {
      const mockExtendedFields: ExtendedFields = {
        INTERNAL_REFERENCE: 1,
        CAPTION: 'Test CAPTION',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockExtendedFields);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/1');
      expect(result).toEqual(mockExtendedFields);
    });

    it('should get extendedFields by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/1?expandLevel=full');
    });

    it('should create an new extendedFields', async () => {
      const newExtendedFields = {
        CAPTION: 'Test CAPTION',
      };
      const createdExtendedFields: ExtendedFields = { ...newExtendedFields, INTERNAL_REFERENCE: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdExtendedFields);

      const result = await client.create(newExtendedFields);

      expect(client['request']).toHaveBeenCalledWith('post', '/extendedFields', newExtendedFields);
      expect(result).toEqual(createdExtendedFields);
    });

    it('should update an extendedFields', async () => {
      const updateExtendedFields = {
        INTERNAL_REFERENCE: 1,
        CAPTION: 'Test CAPTION',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateExtendedFields);

      const result = await client.update(1, updateExtendedFields);
      ...
      expect(result).toEqual(updateExtendedFields);
    });

    it('should patch an extendedFields', async () => {
      const patchData = { CAPTION: 'Updated' };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.patch(1, patchData);

      expect(client['request']).toHaveBeenCalledWith('patch', '/extendedFields/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete an extendedFields', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/extendedFields/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search extendedFields', async () => {
      const criteria = { modulenr: 1 };
  ...
  describe('Analytics Methods', () => {
    it('should get extendedFields analytics', async () => {
    ...
    it('should get extendedFields count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/extendedFields/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: ExtendedFields not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: ExtendedFields not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    it('should call equalizePayAmntV1 with data', async () => {
      const mockData = { MODULENR: 1, LEVEL_: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/EqualizePayAmnt`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });

    it('should call equalizePayAmntV1Get with query options', async () => {
      const options = { limit: 10, fields: ['MODULENR', 'LEVEL_'] as ExtendedFieldsField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/FillSMMACCCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/AttachADespatchByLRef/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/GetRelevantCampaigns`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ImportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ExportToXML/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/CreateCompositeLines`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/FormSeriLotLines/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/FormSeriLotLines/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ApplyRePayPln/1/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ExportToXmlStr/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ExportToXmlStr/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ExportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/extendedFields/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/ApplyADiscount/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/ApplyADiscount/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/extendedFields/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/extendedFields/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
