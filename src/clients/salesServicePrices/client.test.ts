import { SalesServicePricesClient } from './client';
import { SalesServicePrices, SalesServicePricesField, SalesServicePricesSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('SalesServicePricesClient', () => {
  let client: SalesServicePricesClient;
  ...
  beforeEach(() => {
    ...
    client = new SalesServicePricesClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all salesServicePrices', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices');
      expect(result).toEqual(mockResponse);
    });

    it('should get all salesServicePrices with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['CARD_CODE'] as SalesServicePricesSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/salesServicePrices?limit=10&offset=0&sort=CARD_CODE'
      );
    });

    it('should get salesServicePrices by ID', async () => {
      const mockSalesServicePrices: SalesServicePrices = {
        INTERNAL_REFERENCE: 1,
        ACTIVE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockSalesServicePrices);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices/1');
      expect(result).toEqual(mockSalesServicePrices);
    });

    it('should get salesServicePrices by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/salesServicePrices/1?expandLevel=full'
      );
    });

    it('should create a new salesServicePrices', async () => {
      const newSalesServicePrices = {
        ACTIVE: 1,
      };
      const createdSalesServicePrices: SalesServicePrices = {
        ...newSalesServicePrices,
        INTERNAL_REFERENCE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdSalesServicePrices);

      const result = await client.create(newSalesServicePrices);
      ...
      expect(result).toEqual(createdSalesServicePrices);
    });

    it('should update a salesServicePrices', async () => {
      const updateSalesServicePrices = {
        INTERNAL_REFERENCE: 1,
        ACTIVE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateSalesServicePrices);

      const result = await client.update(1, updateSalesServicePrices);

      expect(client['request']).toHaveBeenCalledWith(
        'put',
        '/salesServicePrices/1',
        updateSalesServicePrices
      );
      expect(result).toEqual(updateSalesServicePrices);
    });

    it('should patch a salesServicePrices', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('patch', '/salesServicePrices/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a salesServicePrices', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/salesServicePrices/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search salesServicePrices', async () => {
  ...
  describe('Analytics Methods', () => {
    it('should get salesServicePrices analytics', async () => {
    ...
    it('should get salesServicePrices count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/salesServicePrices/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: SalesServicePrices not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: SalesServicePrices not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    it('should call reCalculateV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ReCalculate`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1Get with query options', async () => {
      const options = {
        limit: 10,
        fields: ['CARD_CODE', 'OWNER_TYPE'] as SalesServicePricesField[],
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/FillAccCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ApplyADiscount/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyADiscount/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/EqualizePayAmnt`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/EqualizePayAmnt`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/FillSMMACCCodes`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/FillSMMACCCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/AttachADespatchByLRef/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DeleteCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/SetClientInfo`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/GetRelevantCampaigns`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ImportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistApplyaccdisttemplateV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/ApplyAccDistTemplate/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistApplyaccdisttemplateV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/ApplyAccDistTemplate/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistSetlinetotalsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/SetLineTotals/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistSetlinetotalsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/SetLineTotals/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getDIVISION_LISTV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/DIVISION_LIST`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getDIVISION_LISTByIdV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAppendline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/AppendLine2`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAppendline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/AppendLine2`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAddserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/AddSeriLots/test/1/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAddserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/AddSeriLots/test/1/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListGetstocklinepriceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/GetStockLinePrice/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListGetstocklinepriceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/GetStockLinePrice/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAddline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/AddLine2/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAddline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/AddLine2/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAppendserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/AppendSeriLots/test/1/1/test/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListAppendserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/AppendSeriLots/test/1/1/test/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListEqualizebalanceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/EqualizeBalance/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListEqualizebalanceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/EqualizeBalance/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListApplyaccdisttemplateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/ApplyAccDistTemplate/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListApplyaccdisttemplateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/ApplyAccDistTemplate/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListSetlinetotalsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/DIVISION_LIST/SetLineTotals/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call divisionListSetlinetotalsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/DIVISION_LIST/SetLineTotals/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ExportToXML/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/CreateCompositeLines`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/FormSeriLotLines/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/FormSeriLotLines/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ApplyRePayPln/1/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ExportToXmlStr/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ExportToXmlStr/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ApplyCondition`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/ExportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getVGENXMLLISTV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/VGENXMLLIST`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getVGENXMLLISTByIdV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/salesServicePrices/1/VGENXMLLIST/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAppendline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/AppendLine2`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAppendline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/AppendLine2`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAddserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/AddSeriLots/test/1/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAddserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/AddSeriLots/test/1/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistGetstocklinepriceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/GetStockLinePrice/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistGetstocklinepriceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/GetStockLinePrice/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAddline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/AddLine2/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAddline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/AddLine2/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAppendserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/AppendSeriLots/test/1/1/test/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistAppendserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/AppendSeriLots/test/1/1/test/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistEqualizebalanceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/EqualizeBalance/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistEqualizebalanceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/EqualizeBalance/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistApplyaccdisttemplateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/ApplyAccDistTemplate/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistApplyaccdisttemplateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/ApplyAccDistTemplate/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistSetlinetotalsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/SetLineTotals/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistSetlinetotalsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/SetLineTotals/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getVGENXMLLISTVALREFSLISTV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getVGENXMLLISTVALREFSLISTByIdV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAppendline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/AppendLine2`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAppendline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/AppendLine2`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAddserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/AddSeriLots/test/1/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAddserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/AddSeriLots/test/1/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistGetstocklinepriceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/GetStockLinePrice/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistGetstocklinepriceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/GetStockLinePrice/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAddline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/AddLine2/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAddline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/AddLine2/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAppendserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/AppendSeriLots/test/1/1/test/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistAppendserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/AppendSeriLots/test/1/1/test/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistEqualizebalanceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/salesServicePrices/1/VGENXMLLIST/1/VALREFSLIST/EqualizeBalance/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call vgenxmllistvalrefslistEqualizebalanceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/salesServicePrices/VGENXMLLIST/1/VALREFSLIST/EqualizeBalance/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
