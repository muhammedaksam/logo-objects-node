import { GlaccountsClient } from './client';
import { Glaccounts, GlaccountsField, GlaccountsSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('GlaccountsClient', () => {
  let client: GlaccountsClient;
  ...
  beforeEach(() => {
    ...
    client = new GlaccountsClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all glaccounts', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts');
      expect(result).toEqual(mockResponse);
    });

    it('should get all glaccounts with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['ACTIVE'] as GlaccountsSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/GLAccounts?limit=10&offset=0&sort=ACTIVE'
      );
    });

    it('should get glaccounts by ID', async () => {
      const mockGlaccounts: Glaccounts = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        DESCRIPTION: 'Test DESCRIPTION',
        ACTIVE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockGlaccounts);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/1');
      expect(result).toEqual(mockGlaccounts);
    });

    it('should get glaccounts by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/1?expandLevel=full');
    });

    it('should create a new glaccounts', async () => {
      const newGlaccounts = {
        CODE: 'Test CODE',
        DESCRIPTION: 'Test DESCRIPTION',
        ACTIVE: 1,
      };
      const createdGlaccounts: Glaccounts = { ...newGlaccounts, INTERNAL_REFERENCE: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdGlaccounts);

      const result = await client.create(newGlaccounts);

      expect(client['request']).toHaveBeenCalledWith('post', '/GLAccounts', newGlaccounts);
      expect(result).toEqual(createdGlaccounts);
    });

    it('should update a glaccounts', async () => {
      ...
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateGlaccounts);

      const result = await client.update(1, updateGlaccounts);

      expect(client['request']).toHaveBeenCalledWith('put', '/GLAccounts/1', updateGlaccounts);
      expect(result).toEqual(updateGlaccounts);
    });

    it('should patch a glaccounts', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('patch', '/GLAccounts/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a glaccounts', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/GLAccounts/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search glaccounts', async () => {
    ...
    it('should search by CL_NAME', async () => {
      await client.searchByClName('test');
      expect(client.getAll).toHaveBeenCalledWith({ q: "CL_NAME like 'test*'" });
    });
  ...
  describe('Analytics Methods', () => {
    it('should get glaccounts analytics', async () => {
    ...
    it('should get glaccounts count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/GLAccounts/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: Glaccounts not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: Glaccounts not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });

    it('should call importFromXmlStrV1 with query options', async () => {
      const options = { limit: 10, fields: ['ACTIVE', 'CODE'] as GlaccountsField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ExportToXmlStr/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ExportToXmlStr/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ExportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/GLAccounts/ReCalculate`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/GLAccounts/FillAccCodes`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ApplyADiscount/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ApplyADiscount/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/AttachADespatchByLRef/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/GLAccounts/SetClientInfo`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/GetRelevantCampaigns`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ImportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getDEFNFLDSLISTV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/DEFNFLDSLIST`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getDEFNFLDSLISTByIdV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/DEFNFLDSLIST/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAppendline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/AppendLine2`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAppendline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/AppendLine2`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAddserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/AddSeriLots/test/1/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAddserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/AddSeriLots/test/1/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistGetstocklinepriceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/GetStockLinePrice/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistGetstocklinepriceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/GetStockLinePrice/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAddline2V1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/AddLine2/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAddline2V1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/AddLine2/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAppendserilotsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/AppendSeriLots/test/1/1/test/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistAppendserilotsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/AppendSeriLots/test/1/1/test/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistEqualizebalanceV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/EqualizeBalance/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistEqualizebalanceV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/EqualizeBalance/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistApplyaccdisttemplateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/ApplyAccDistTemplate/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistApplyaccdisttemplateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/ApplyAccDistTemplate/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistSetlinetotalsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/DEFNFLDSLIST/SetLineTotals/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call defnfldslistSetlinetotalsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/DEFNFLDSLIST/SetLineTotals/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ExportToXML/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/CreateCompositeLines`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/FormSeriLotLines/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/GLAccounts/ApplyCampaign`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/GLAccounts/1/ApplyRePayPln/1/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/GLAccounts/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/GLAccounts/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
