...
import { ShiftAssignments, ShiftAssignmentsField, ShiftAssignmentsSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('ShiftAssignmentsClient', () => {
  let client: ShiftAssignmentsClient;
  ...
  beforeEach(() => {
    ...
    client = new ShiftAssignmentsClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all shiftAssignments', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments');
      expect(result).toEqual(mockResponse);
    });

    it('should get all shiftAssignments with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['SOURCE_TYPE'] as ShiftAssignmentsSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/shiftAssignments?limit=10&offset=0&sort=SOURCE_TYPE'
      );
    });

    it('should get shiftAssignments by ID', async () => {
      const mockShiftAssignments: ShiftAssignments = {
        INTERNAL_REFERENCE: 1,
        SOURCE_TYPE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockShiftAssignments);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/1');
      expect(result).toEqual(mockShiftAssignments);
    });

    it('should get shiftAssignments by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/1?expandLevel=full');
    });

    it('should create a new shiftAssignments', async () => {
      const newShiftAssignments = {};
      const createdShiftAssignments: ShiftAssignments = {
        ...newShiftAssignments,
        INTERNAL_REFERENCE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdShiftAssignments);

      const result = await client.create(newShiftAssignments);
      ...
      expect(result).toEqual(createdShiftAssignments);
    });

    it('should update a shiftAssignments', async () => {
      const updateShiftAssignments = {
        INTERNAL_REFERENCE: 1,
        SOURCE_TYPE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateShiftAssignments);

      const result = await client.update(1, updateShiftAssignments);
      ...
      expect(result).toEqual(updateShiftAssignments);
    });

    it('should patch a shiftAssignments', async () => {
      const patchData = { SOURCE_TYPE: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.patch(1, patchData);

      expect(client['request']).toHaveBeenCalledWith('patch', '/shiftAssignments/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a shiftAssignments', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/shiftAssignments/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search shiftAssignments', async () => {
      const criteria = { sourceType: 1 };
      await client.search(criteria);

      expect(client.getAll).toHaveBeenCalled();
    });

    it('should search by SOURCE_CODE', async () => {
      await client.searchBySourceCode('test');
      expect(client.getAll).toHaveBeenCalledWith({ q: "SOURCE_CODE like 'test*'" });
    });

    it('should search by SOURCE_TYPE', async () => {
      await client.searchBySourceType('test');
      expect(client.getAll).toHaveBeenCalledWith({ q: expect.any(String) });
    });
  });

  describe('Analytics Methods', () => {
    it('should get shiftAssignments analytics', async () => {
    ...
    it('should get shiftAssignments count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/shiftAssignments/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: ShiftAssignments not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: ShiftAssignments not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    it('should call equalizePayAmntV1 with data', async () => {
      const mockData = { SOURCE_TYPE: 1, SOURCE_CODE: 'test' };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/EqualizePayAmnt`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1Get with query options', async () => {
      const options = {
        limit: 10,
        fields: ['SOURCE_TYPE', 'SOURCE_CODE'] as ShiftAssignmentsField[],
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/FillSMMACCCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/AttachADespatchByLRef/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/DeleteCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/SetClientInfo`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/GetRelevantCampaigns`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ImportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ExportToXML/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/CreateCompositeLines`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/FormSeriLotLines/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/FormSeriLotLines/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ApplyRePayPln/1/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ExportToXmlStr/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ExportToXmlStr/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyCondition`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ExportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/shiftAssignments/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/ApplyADiscount/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/ApplyADiscount/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/shiftAssignments/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/shiftAssignments/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
