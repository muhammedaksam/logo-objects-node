...
import { Districts, DistrictsField, DistrictsSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('DistrictsClient', () => {
  let client: DistrictsClient;
  ...
  beforeEach(() => {
    ...
    client = new DistrictsClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all districts', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/districts');
      expect(result).toEqual(mockResponse);
    });

    it('should get all districts with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['INFO_TOWNNR'] as DistrictsSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/districts?limit=10&offset=0&sort=INFO_TOWNNR'
      );
    });

    it('should get districts by ID', async () => {
      const mockDistricts: Districts = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockDistricts);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/districts/1');
      expect(result).toEqual(mockDistricts);
    });

    it('should get districts by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith('get', '/districts/1?expandLevel=full');
    });

    it('should create a new districts', async () => {
      const newDistricts = {
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      const createdDistricts: Districts = { ...newDistricts, INTERNAL_REFERENCE: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdDistricts);

      const result = await client.create(newDistricts);

      expect(client['request']).toHaveBeenCalledWith('post', '/districts', newDistricts);
      expect(result).toEqual(createdDistricts);
    });

    it('should update a districts', async () => {
      const updateDistricts = {
        INTERNAL_REFERENCE: 1,
        CODE: 'Test CODE',
        NAME: 'Test NAME',
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateDistricts);

      const result = await client.update(1, updateDistricts);

      expect(client['request']).toHaveBeenCalledWith('put', '/districts/1', updateDistricts);
      expect(result).toEqual(updateDistricts);
    });

    it('should patch a districts', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('patch', '/districts/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a districts', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/districts/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search districts', async () => {
  ...
  describe('Analytics Methods', () => {
    it('should get districts analytics', async () => {
    ...
    it('should get districts count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/districts/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/districts/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/districts/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/districts/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: Districts not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: Districts not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    ...
    it('should call equalizePayAmntV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });

    it('should call equalizePayAmntV1Get with query options', async () => {
      const options = { limit: 10, fields: ['INFO_TOWNNR', 'CODE'] as DistrictsField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/AttachADespatchByLRef/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/DeleteCampaign`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/SetClientInfo`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/GetRelevantCampaigns`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ImportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ExportToXML/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/CreateCompositeLines`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/FormSeriLotLines/1`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/ApplyCampaign`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ApplyRePayPln/1/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ExportToXmlStr/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/ApplyCondition`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ExportImage/test/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/ReCalculate`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('post', `/districts/FillAccCodes`, mockData);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/districts/1/ApplyADiscount/test`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/districts/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/districts/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
