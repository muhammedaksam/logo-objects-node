import { FaassignmentslipsClient } from './client';
import { Faassignmentslips, FaassignmentslipsField, FaassignmentslipsSortSpec } from './types';
import { ApiClientConfig } from '../../types';

describe('FaassignmentslipsClient', () => {
  let client: FaassignmentslipsClient;
  ...
  beforeEach(() => {
    ...
    client = new FaassignmentslipsClient(config);
  });
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should get all faassignmentslips', async () => {
      const mockResponse = {
        items: [
          {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips');
      expect(result).toEqual(mockResponse);
    });

    it('should get all faassignmentslips with options', async () => {
      const options = { limit: 10, offset: 0, sort: ['PERREF'] as FaassignmentslipsSortSpec };
      await client.getAll(options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/FAAssignmentSlips?limit=10&offset=0&sort=PERREF'
      );
    });

    it('should get faassignmentslips by ID', async () => {
      const mockFaassignmentslips: Faassignmentslips = {
        INTERNAL_REFERENCE: 1,
        PERREF: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(mockFaassignmentslips);

      const result = await client.getById(1);

      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips/1');
      expect(result).toEqual(mockFaassignmentslips);
    });

    it('should get faassignmentslips by ID with options', async () => {
      const options = { expandLevel: 'full' };
      await client.getById(1, options);

      expect(client['request']).toHaveBeenCalledWith(
        'get',
        '/FAAssignmentSlips/1?expandLevel=full'
      );
    });

    it('should create a new faassignmentslips', async () => {
      const newFaassignmentslips = {};
      const createdFaassignmentslips: Faassignmentslips = {
        ...newFaassignmentslips,
        INTERNAL_REFERENCE: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(createdFaassignmentslips);

      const result = await client.create(newFaassignmentslips);
      ...
      expect(result).toEqual(createdFaassignmentslips);
    });

    it('should update a faassignmentslips', async () => {
      const updateFaassignmentslips = {
        INTERNAL_REFERENCE: 1,
        PERREF: 1,
      };
      jest.spyOn(client, 'request' as any).mockResolvedValue(updateFaassignmentslips);

      const result = await client.update(1, updateFaassignmentslips);

      expect(client['request']).toHaveBeenCalledWith(
        'put',
        '/FAAssignmentSlips/1',
        updateFaassignmentslips
      );
      expect(result).toEqual(updateFaassignmentslips);
    });

    it('should patch a faassignmentslips', async () => {
      const patchData = { PERREF: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.patch(1, patchData);

      expect(client['request']).toHaveBeenCalledWith('patch', '/FAAssignmentSlips/1', patchData);
      expect(result).toEqual({ success: true });
    });

    it('should delete a faassignmentslips', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.delete(1);

      expect(client['request']).toHaveBeenCalledWith('delete', '/FAAssignmentSlips/1');
      expect(result).toEqual({ success: true });
    });
  });
  ...
  describe('Search Methods', () => {
    ...
    it('should search faassignmentslips', async () => {
      const criteria = { perref: 1 };
  ...
  describe('Analytics Methods', () => {
    it('should get faassignmentslips analytics', async () => {
    ...
    it('should get faassignmentslips count', async () => {
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/FAAssignmentSlips/checktrack');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      const apiError = new Error('API Error: Faassignmentslips not found');
      jest.spyOn(client, 'request' as any).mockRejectedValue(apiError);

      await expect(client.getById(999)).rejects.toThrow('API Error: Faassignmentslips not found');
    });
  ...
  describe('Custom Endpoint Methods', () => {
    it('should call equalizePayAmntV1 with data', async () => {
      const mockData = { PERREF: 1, EMPGRPREF: 1 };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/EqualizePayAmnt`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call equalizePayAmntV1Get', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/EqualizePayAmnt`);
      expect(result).toEqual({ success: true });
    });

    it('should call equalizePayAmntV1Get with query options', async () => {
      const options = { limit: 10, fields: ['PERREF', 'EMPGRPREF'] as FaassignmentslipsField[] };
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });
    ...
    it('should call addSeriLotsForKsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/AddSeriLotsForKs/test/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call addSeriLotsForKsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/AddSeriLotsForKs/test/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/FillSMMACCCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillSMMACCCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/FillSMMACCCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/AttachADespatchByLRef/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByLRefV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/AttachADespatchByLRef/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/AttachADespatchByFicheNo/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call attachADespatchByFicheNoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/AttachADespatchByFicheNo/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/DeleteCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call deleteCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/DeleteCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/SetClientInfo`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setClientInfoV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/SetClientInfo`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/GetRelevantCampaigns`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call getRelevantCampaignsV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/GetRelevantCampaigns`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ApplyCampaignSpecific/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignSpecificV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyCampaignSpecific/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ImportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ImportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ExportBase64EncodedImage/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ExportBase64EncodedImage/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ImportBase64EncodedImage/test/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importBase64EncodedImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ImportBase64EncodedImage/test/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call postReadByIdWithBodyParametersV1 with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/1/readwithbodyparamters`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ExportToXML/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXMLV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ExportToXML/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ImportFromXMLFile/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXMLFileV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ImportFromXMLFile/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/CreateCompositeLines`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call createCompositeLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/CreateCompositeLines`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/FormSeriLotLines/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call formSeriLotLinesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/FormSeriLotLines/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/ApplyCampaign`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyCampaignV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyCampaign`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ApplyRePayPln/1/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyRePayPln/1/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ApplyRePayPlnForInv/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyRePayPlnForInvV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyRePayPlnForInv/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ImportFromXmlStr/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call importFromXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ImportFromXmlStr/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ExportToXmlStr/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportToXmlStrV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ExportToXmlStr/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/ApplyCondition`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyConditionV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyCondition`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ExportImage/test/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call exportImageV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ExportImage/test/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call reCalculateV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/ReCalculate`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', `/FAAssignmentSlips/1/FillAccCodes`);
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call fillAccCodesV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/FillAccCodes`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/ApplyADiscount/test`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call applyADiscountV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/ApplyADiscount/test`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'get',
        `/FAAssignmentSlips/1/SetDefIntValue/test/1/1/1`
      );
      expect(result).toEqual({ success: true });
    });
    ...
    it('should call setDefIntValueV1Post with data', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith(
        'post',
        `/FAAssignmentSlips/SetDefIntValue/test/1/1/1`,
        mockData
      );
      expect(result).toEqual({ success: true });
    });
