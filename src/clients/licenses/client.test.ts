...
describe('LicensesClient', () => {
  ...
  beforeEach(() => {
    ...
    client = new LicensesClient(config);
  });
  ...
  describe('Utility Methods', () => {
    ...
    it('should get database column properties', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/licenses/dbcolumns');
    ...
    it('should track changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/licenses/track');
    ...
    it('should untrack changes', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/licenses/untrack');
    ...
    it('should check track status', async () => {
      ...
      expect(client['request']).toHaveBeenCalledWith('get', '/licenses/checktrack');
      expect(result).toBe(true);
    });
  });

  describe('Custom Endpoint Methods', () => {
    it('should call authorization_V1_licenses', async () => {
      jest.spyOn(client, 'request' as any).mockResolvedValue({ success: true });

      const result = await client.authorization_V1_licenses(1, 'test');

      expect(client['request']).toHaveBeenCalledWith('get', `/licenses/1/type/test`);
